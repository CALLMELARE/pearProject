      ...
      >
        ...
        >
          <div class="muted flex ant-row-flex-middle">
            <span href="javascript:;" class="text-default"
              >{{ organization.name }}
            </span>
            <a-icon
              v-show="currentOrganization.code == organization.code"
              class="muted m-l-lg"
              type="check"
            />
          </div>
...
export default {
  ...
  methods: {
    organizationAction(action) {
      let app = this;
      changeCurrentOrganization(app.organizationList[action.key]).then(
        (res) => {
          if (checkResponse(res)) {
            app.$router.replace(
              "/home/" + app.organizationList[action.key].code
            );
          }
          console.log(res);
        }
